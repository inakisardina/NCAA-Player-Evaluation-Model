# 1. Ensure consistent year type in all sources
...
# 4. Career-level (Overall) aggregation directly from pitch-level data
pitching_combined_overall <- pitching_combined_yearly %>%
  ...
  summarise(
    ...
    avg_ContactPlus = round(weighted.mean(avg_ContactPlus, total_pitches, na.rm = TRUE),1),
...
# 6. Percentile ranks within year groups
